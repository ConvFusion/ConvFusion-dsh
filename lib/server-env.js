 ...
 */
export const BUILTIN_SERVER_URL = {
 ...
 */
export function serverEnvFileCandidates(env = process.env) {
 ...
 */
export function resolveEnvironmentName(env = process.env, fileEnvironment) {
    return (normalizeEnvironment(env[SERVER_ENV_ENV] ?? '') ??
...
function parseServerEnvFile(path) {
    const empty = { environments: {}, dev: {} };
    let raw;
    try {
    ...
    for (const [key, value] of Object.entries(asRecord(obj.environments))) {
        ...
        if (url)
            environments[name] = { serverUrl: url };
    }
    ...
    if (serverDir)
        dev.serverDir = isAbsolute(serverDir) ? serverDir : resolve(base, serverDir);
 ...
 */
export function loadServerEnv(env = process.env, options = {}) {
    ...
    if (!options.fresh && cache.key === key)
        return cache.value;
...
export function environmentServerUrl(env = process.env, options = {}) {
 ...
 */
export function serverUrlPresets(env = process.env, options = {}) {
    const cfg = loadServerEnv(env, options);
    return {
 ...
 */
export function serverAddressKey(raw) {
 ...
 */
export function serverSlotOf(url, env = process.env, options = {}) {
    const target = serverAddressKey(url);
    const presets = serverUrlPresets(env, options);
    if (target === serverAddressKey(presets.development))
        return 'development';
    if (target === serverAddressKey(presets.production))
        return 'production';
    return isLocalServerUrl(url) ? 'development' : 'production';
}
/** 开发流程用值的读取（脚本用；`env` 覆盖文件）。 */
export function resolveDevTooling(env = process.env) {
    ...
    return {
        ...
        port: Number(env.CONVFUSION_TEST_PORT ?? '') ||
            cfg.dev.port ||
            8123,
    };
}
 ...
 * ```
 */
export function serverUrlMismatch(environment, url, env = process.env) {
