...
import { isResearchWorkspace, resolveWorkspace } from './research/workspace.js'
...
export function apply(ctx: Context, rawConfig: Partial<ConfigShape> = {}): void {
  ...
  //
  // ⚠️ 配置必须**可被设置页改写**：用户在【设置】-【ConvFusion】-【本地设置】里改文件名后，
  ...
  // v2 无「锚文件」概念：一个 workspace = 一个研究；非研究目录会被
  // `isResearchWorkspace` 判否，从而不注入任何研究上下文（会话退化为普通助手）。
  // workspace = **当前工作区**：从会话 header.cwd 解析（由 agent/pre-step 持续同步）。
  // 装配阶段还拿不到会话，因此先用 process.cwd() 作为占位，首个 pre-step 即被替换。
  let currentCwd = process.cwd()
  const resolveCurrentWorkspace = (): string => resolveWorkspace(currentCwd, undefined)
  ...
  const research = new ResearchContextService(ctx, resolveCurrentWorkspace, skillContent)
  ctx.effect(() => {
  ...
  ctx.on('agent/pre-step', (payload, next) => {
    const p = payload as unknown as { agent?: { session?: { header?: { cwd?: string } } } }
  ...
  ctx.effect(() => {
    const disposers = defineResearchTools(resolveCurrentWorkspace, {
