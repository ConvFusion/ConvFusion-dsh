...
export function buildSignalContext(workspace: string): SignalContext {
  ...
  let fulltextCount = 0
  try {
    fulltextCount = readdirSync(join(workspace, FULLTEXT_DIR)).filter((f) =>
      /\.(pdf|html|txt)$/i.test(f),
    ).length
  } catch {
    fulltextCount = 0
  }
