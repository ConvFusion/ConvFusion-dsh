 ...
 * 缺哪个映射就在注释里说清，而不是硬凑。
 */
export const PROCESS_STAGE_TO_SERVER_STAGE = {
...
function dimension(state, name) {
    const raw = state?.dimensions?.[name];
...
export function buildPublishContent(input) {
    ...
    const questions = project?.questions && project.questions.length > 0
    ...
    const content = {
        ...
        extensions: {
            convfusion: {
                ...
                maturity: Object.fromEntries(input.progress.progress.dimensions.map((d) => [d.dimension, d.level])),
    ...
    if (!coreIdea)
        missing.push('core_idea');
    if (!methodOverview)
        missing.push('method_overview');
    if (!fields.length)
        missing.push('research_fields');
    return { content, missing };
}
//# sourceMappingURL=publish-content.js.map