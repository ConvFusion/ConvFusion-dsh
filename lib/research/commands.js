 ...
 */
function buildKickoffText(workspace, topic) {
    void workspace;
    return [
        `开始这项研究：${topic}`,
        '',
 ...
 */
function buildContinueText(workspace, intent) {
    const project = loadProjectFile(workspace);
    ...
    if (plans.length > 0) {
        ...
        for (const p of plans)
            lines.push(`  - \`${p.path}\` [${p.status ?? 'draft'}] — ${p.title}`);
    }
    lines.push('');
    lines.push('Read the research workspace as needed (`project.md`, `research-state.md`, `plans/`, `papers/`, `research/`). ' +
 ...
 */
export function defineResearchCommand(ctx, resolveCurrentWorkspace) {
