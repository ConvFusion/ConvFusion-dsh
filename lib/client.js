window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var en = {
		  ...
		  "community.exchange.downloadStarted": "Download started ({files} files, {size}).",
		  "community.exchange.noFilesYet": "Nothing to download on the server yet.",
		    ...
		    async (endpoint, payload) => {
		      try {
		        if (endpoint === "state") {
		          ...
		          setStaleHost(outcome.state.protocol !== 14);
		...
		function readProgressValue(res) {
		  ...
		  if (value.protocol !== 14) return { kind: "hidden" };
