...
export interface UploadCategory {
  ...
  bytes: number
  /**
   * 为什么这样判（i18n key，客户端翻译）。
   * 分类 ID 与 reason key 一一对应：`upload.category.<id>` / `upload.reason.<id>`。
   */
}
 ...
 */
export function classify(relPath: string, size: number): { categoryId: string; decision: UploadDecision } {
  const p = norm(relPath)
  const parts = p.split('/')

  // ① 机器产物：任何一层是构建/依赖目录，或文件名/扩展名属于机器产物
  if (parts.some((seg) => JUNK_DIRS.has(seg))) return { categoryId: 'build-artifacts', decision: 'excluded' }
  if (JUNK_NAME.test(parts[parts.length - 1] ?? '')) return { categoryId: 'build-artifacts', decision: 'excluded' }
  if (BUILD_EXTS.has(ext(p))) return { categoryId: 'build-artifacts', decision: 'excluded' }
  if (WEIGHT_EXTS.has(ext(p))) return { categoryId: 'models-data', decision: 'excluded' }
  // `harness/` 是 DSH 运行期产物（会话/缓存），不属于研究内容
  if (under(p, 'harness')) return { categoryId: 'runtime-artifacts', decision: 'excluded' }

  // ② 原始素材 / 大文件：有价值但默认不传
  if (under(p, 'research/literature/fulltext')) return { categoryId: 'literature-fulltext', decision: 'optional' }
  ...
  if (size >= LARGE_FILE_BYTES) return { categoryId: 'large-files', decision: 'optional' }

  // ③ 研究资产：推荐
  if (p === 'project.md' || p === 'research-state.md') return { categoryId: 'state', decision: 'recommended' }
  /*
   ...
   * 导师【上传】指导结果走的是另一条通道（`scanReviewFiles` 直接列 `review/` 下的文件），
   * **不经过这里**，所以反向流程不受影响。这一条不对称是刻意的，有断言钉住。
   */
  if (under(p, 'review')) return { categoryId: 'review', decision: 'excluded' }
  if (under(p, 'plans')) return { categoryId: 'plans', decision: 'recommended' }
  if (under(p, 'papers')) return { categoryId: 'papers', decision: 'recommended' }
  if (under(p, 'experiments')) return { categoryId: 'experiments', decision: 'recommended' }
  if (under(p, 'outputs')) return { categoryId: 'outputs', decision: 'recommended' }
  if (under(p, 'research')) return { categoryId: 'research-assets', decision: 'recommended' }
...
export function isSelectable(decision: UploadDecision): boolean {
...
function measureDir(dir: string, budget = 20000): { bytes: number; files: number } {
  let bytes = 0
  let files = 0
 ...
 */
export function buildUploadPlan(root: string): UploadPlan {
  const files: Array<{ relPath: string; size: number; categoryId: string; decision: UploadDecision }> = []
  /** 整枝跳过的机器产物目录：按**分类**累计体积/计数（分类行也要算得平）。 */
  const skipped = new Map<string, { bytes: number; files: number }>()
  ...
  const byCategory = new Map<string, UploadCategory>()
  for (const f of files) {
    ...
    if (!isPlaceholder) {
      ...
      if (extra) cat.bytes += extra.bytes
    }
    byCategory.set(f.categoryId, cat)
  }

  const categories = [...byCategory.values()].sort(
    (a, b) => CATEGORY_ORDER.indexOf(a.id as never) - CATEGORY_ORDER.indexOf(b.id as never),
  )
  for (const c of categories) c.files.sort((a, b) => a.relPath.localeCompare(b.relPath))

  const sum = (d: UploadDecision): { bytes: number; files: number } => {
    let bytes = 0
    let files = 0
    for (const c of categories) {
      if (c.decision !== d) continue
      bytes += c.bytes
  ...
  for (const c of categories) {
    if (c.decision !== 'recommended') continue
    for (const f of c.files) {
      if (f.relPath.endsWith('/')) continue
...
export function planUploadBatches(
...
): { batches: string[][]; skipped: OversizeFile[]; bytes: number } {
  const skipped: OversizeFile[] = []
  ...
  for (const relPath of [...selection].sort()) {
    ...
    if (
      current.length >= UPLOAD_LIMITS.maxFilesPerRequest ||
