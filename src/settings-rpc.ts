...
export function createSettingsRpcHandler(
   ...
   */
  const credentialPatch = (config: Config, base: string, apiKey: string): Partial<Config> => {
    const slot = serverSlotOf(base, env())
    const legacy = (config.convfusionApiKey ?? '').trim()
    const migration: Partial<Config> = {}
    if (legacy) {
      if (!(config.convfusionDevApiKey ?? '').trim()) migration.convfusionDevApiKey = legacy
