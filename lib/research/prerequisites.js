 ...
 * 拼在前，故用户的块天然优先。
 */
export function parsePrerequisites(content) {
    if (!content)
        return undefined;
    const out = [];
    for (const raw of content.split(/\r?\n/)) {
        ...
        if (!m) {
            ...
            if (out.length > 0 && (line.startsWith('```') || line !== ''))
                break;
            continue;
        }
        const body = m[1];
...
export function isKnownSignal(signal) {
    return STAGE_SIGNALS.includes(signal);
}
/**
 * 评估一组技能的前置依赖。
 *
 ...
 */
export function assessPrerequisites(skills, ctx) {
    const out = [];
    for (const { id, content } of skills) {
        ...
        for (const line of prerequisites) {
            ...
            if (!lineSatisfied)
                unmet.push({ signals: line.signals, note: line.note, missingSignals });
        }
