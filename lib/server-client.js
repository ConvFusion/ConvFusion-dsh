...
export async function fetchProjectArchive(base, apiKey, projectId, options = {}) {
    ...
    if (!id)
        throw new ServerError('bad-request', '缺少 projectId。');
    const bytes = await fetchBinary(base, `/projects/${encodeURIComponent(id)}/files/archive`, key, options, '下载工作区快照');
    return bytes;
}
/**
...
export async function fetchProjectFileBytes(base, apiKey, projectId, fileId, options = {}) {
    ...
    return await fetchBinary(base, `/projects/${encodeURIComponent(id)}/files/${encodeURIComponent(fid)}`, key, options, '下载文件');
}
